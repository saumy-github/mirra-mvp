...
def create_realistic_tshirt():
    ...
    # Dimensions from XML specification (in meters)
    body_width = 0.50         # HalfChestWidth: 50cm
    body_length = 0.70        # GarmentLength: 70cm
    body_depth = 0.18         # Front-to-back depth (estimated)
    
    waist_scale = 0.96        # Waist slightly narrower
    hem_scale = 1.00          # HemWidth matches chest: 50cm
    
    neck_radius = 0.09        # NeckWidth: 18cm (radius = 9cm)
    neck_depth_front = 0.09   # Neck.front: 9cm
    neck_depth_back = 0.03    # Neck.back: 3cm
    
    sleeve_length = 0.22      # SleeveLength: 22cm
    bicep_width = 0.36        # BicepWidth: 36cm
    sleeve_radius_shoulder = bicep_width / (2 * math.pi)  # Circumference to radius
    sleeve_radius_end = 0.07  # 7cm at wrist
    
    armhole_depth = 0.22      # ArmholeDepth: 22cm
    shoulder_width = 0.44     # ShoulderWidth: 44cm
    neckband_height = 0.04    # BandHeight: 4cm
    ...
    shoulder_height = body_length - 0.03
    ...
    bpy.ops.mesh.remove_doubles(threshold=0.02)  # Increased threshold to catch sleeve vertices
