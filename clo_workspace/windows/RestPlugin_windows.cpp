...
{
    // MIRRA_PLUGIN_LOG_DIR is baked in by build_plugin.py's
    // generate_build_info_header() from LOGS_DIR = Path(__file__).resolve().parent
    // / "logs" — i.e. clo_workspace/logs on whichever machine ran the build.
    // Computed fresh per build, per machine; never hand-typed.
...
// runs every 200 ms inside QueueDrainTimer to re-chain from the new proc so our
...
VOID CALLBACK QueueDrainTimer(HWND, UINT, UINT_PTR, DWORD)
{
    ...
    // status). Throttled to every ~100th tick (~20s at 200ms) so the log
    ...
    for (auto& cmd : batch) {
        ...
        try {
            ...
            // ── Native avatar import (.avt) ───────────────────────────────
                ...
                TraceLog("END   import-pattern success=" + std::string(asyncResult.success ? "true" : "false")
            ...
            // ── Create seam ───────────────────────────────────────────────
                ...
                TraceLog("END   create-seam success=" + std::string(asyncResult.success ? "true" : "false"));
                asyncResult.message = asyncResult.success
            ...
            // param1 = texture file path (PNG/JPEG); param3 = pattern_index
                ...
                TraceLog("BEGIN set-fabric-texture pattern=" + std::to_string(cmd.param3) +
                    " path=" + cmd.param1);
                std::string guardErr;
                if (!CLOGuard::fabricReady(guardErr)       ||
                    !CLOGuard::patternIndexValid(cmd.param3, guardErr) ||
                ...
                TraceLog("END   set-fabric-texture success=" + std::string(asyncResult.success ? "true" : "false"));
            }
            // ── Set fabric graphic overlay ────────────────────────────────────
            ...
            // Dispatched identically to set-fabric-texture (base texture replacement).
                ...
                TraceLog("BEGIN set-fabric-graphic pattern=" + std::to_string(cmd.param3) +
                    " path=" + cmd.param1);
                std::string guardErr;
                if (!CLOGuard::fabricReady(guardErr)       ||
                    !CLOGuard::patternIndexValid(cmd.param3, guardErr) ||
                ...
                TraceLog("END   set-fabric-graphic success=" + std::string(asyncResult.success ? "true" : "false"));
            }
            // ── Save project ──────────────────────────────────────────────
