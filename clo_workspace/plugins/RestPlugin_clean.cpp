    ...
    while (!g_commandQueue.empty()) {
        ...
        try {
            if (cmd.type == "import-avatar") {
                ...
                std::string path = cmd.param1;
                std::replace(path.begin(), path.end(), '/', '\\');
                
                Marvelous::ImportDxfOption options;
                options.m_bAppend = true;  // Add to existing patterns
                options.m_bAutoScale = false;  // Don't auto-scale
                options.m_Scale = 1.0f;  // Use original scale
                options.m_Angle = 0.0f;
                options.m_bImportPatternAnnotation = true;
                options.m_bAutoTraceAsInternalLines = false;
                options.m_SwapCuttingLineAndSewingLine = false;
                options.m_bIncludeNotchs = true;
                options.m_bAutoDistribute = false;
                
                bool success = IMPORT_API->ImportDXF(path, options);
                if (success) {
                    results += "✓ Imported pattern: " + path + "\n";
                    success_count++;
                } else {
                    results += "✗ Failed to import pattern: " + path + "\n";
                    results += "  (Check: File exists? Valid DXF? Correct units?)\n";
                    fail_count++;
                }
            }
        }
