...
from . import service
from .models import CaptureSessionDocument


def shape_session(session: CaptureSessionDocument, *, include_token: bool = False) -> dict:
    shaped = {
        "sessionId": session.id,
        "state": session.state,
        "manualCode": session.manual_code,
        "photoUploaded": session.photo is not None,
        "avatarJobId": session.avatar_job_id,
