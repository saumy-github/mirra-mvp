...
**Status (2026-07-20): Phases 1–8 are implemented in `website/backend/` and verified end-to-end against the real `mirratest` database** (per-phase check scripts + `scripts/smoke_e2e.py`, which runs the full pilot flow: guest → measurements → capture → demo avatar → catalog → demo try-on → look → analytics → cascade deletion). Phase 9 (frontend swap-over) is not started. Engine modes are demo-only until the CLO3D worker/queue decision lands (Phase 0, item 3).
